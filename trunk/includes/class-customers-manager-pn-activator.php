...
class CUSTOMERS_MANAGER_PN_Activator {
   ...
   * Functions to be loaded on plugin activation. This actions creates roles, options and post information attached to the plugin.
    ...
    if (empty(get_posts(['fields' => 'ids', 'numberposts' => -1, 'post_type' => 'cm_pn_funnel', 'post_status' => 'any', ]))) {
      ...
      $customers_manager_pn_id = $post_functions->customers_manager_pn_insert_post(esc_html($customers_manager_pn_title), $customers_manager_pn_post_content, '', sanitize_title(esc_html($customers_manager_pn_title)), 'cm_pn_funnel', 'publish', 1);
        ...
        if (!empty($locales)) {
          foreach ($locales as $locale) {
            if ($locale != $language) {
              ...
              $customers_manager_pn_post_content = '';
              $translated_customers_manager_pn_id = $post_functions->customers_manager_pn_insert_post(esc_html($customers_manager_pn_title), $customers_manager_pn_post_content, '', sanitize_title(esc_html($customers_manager_pn_title)), 'cm_pn_funnel', 'publish', 1);
