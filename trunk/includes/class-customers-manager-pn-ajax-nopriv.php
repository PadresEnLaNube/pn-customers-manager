...
class CUSTOMERS_MANAGER_PN_Ajax_Nopriv {
  ...
  public function customers_manager_pn_ajax_nopriv_server() {
    if (array_key_exists('customers_manager_pn_ajax_nopriv_type', $_POST)) {
      ...
      $customers_manager_pn_ajax_nopriv_type = cm_pn_forms::customers_manager_pn_sanitizer(wp_unslash($_POST['customers_manager_pn_ajax_nopriv_type']));
          ...
          if ($customers_manager_pn_key['multiple'] == 'true') {
            ...
            if (!empty($_POST[$customers_manager_pn_clear_key])) {
              ...
              $sanitized_array = array_map(function($value) use ($customers_manager_pn_key) {
                return cm_pn_forms::customers_manager_pn_sanitizer(
                  $value,
      ...
      switch ($customers_manager_pn_ajax_nopriv_type) {
        case 'cm_pn_form_save':
          $cm_pn_form_type = !empty($_POST['cm_pn_form_type']) ? cm_pn_forms::customers_manager_pn_sanitizer(wp_unslash($_POST['cm_pn_form_type'])) : '';

          if (!empty($customers_manager_pn_key_value) && !empty($cm_pn_form_type)) {
            $cm_pn_form_id = !empty($_POST['cm_pn_form_id']) ? cm_pn_forms::customers_manager_pn_sanitizer(wp_unslash($_POST['cm_pn_form_id'])) : 0;
            $cm_pn_form_subtype = !empty($_POST['cm_pn_form_subtype']) ? cm_pn_forms::customers_manager_pn_sanitizer(wp_unslash($_POST['cm_pn_form_subtype'])) : '';
            $user_id = !empty($_POST['cm_pn_form_user_id']) ? cm_pn_forms::customers_manager_pn_sanitizer(wp_unslash($_POST['cm_pn_form_user_id'])) : 0;
            $post_id = !empty($_POST['cm_pn_form_post_id']) ? cm_pn_forms::customers_manager_pn_sanitizer(wp_unslash($_POST['cm_pn_form_post_id'])) : 0;
            $post_type = !empty($_POST['cm_pn_form_post_type']) ? cm_pn_forms::customers_manager_pn_sanitizer(wp_unslash($_POST['cm_pn_form_post_type'])) : '';
            ...
            if (($cm_pn_form_type == 'user' && empty($user_id) && !in_array($cm_pn_form_subtype, ['user_alt_new'])) || ($cm_pn_form_type == 'post' && (empty($post_id) && !(!empty($cm_pn_form_subtype) && in_array($cm_pn_form_subtype, ['post_new', 'post_edit'])))) || ($cm_pn_form_type == 'option' && !is_user_logged_in())) {
              ...
              $_SESSION['cm_pn_form'][$cm_pn_form_id]['values'] = $customers_manager_pn_key_value;
              ...
              switch ($cm_pn_form_type) {
                case 'user':
                  if (!in_array($cm_pn_form_subtype, ['user_alt_new'])) {
                    if (empty($user_id)) {
                      if (CUSTOMERS_MANAGER_PN_Functions_User::customers_manager_pn_user_is_admin(get_current_user_id())) {
                        $user_login = !empty($_POST['user_login']) ? cm_pn_forms::customers_manager_pn_sanitizer(wp_unslash($_POST['user_login'])) : 0;
                        $user_password = !empty($_POST['user_password']) ? cm_pn_forms::customers_manager_pn_sanitizer(wp_unslash($_POST['user_password'])) : 0;
                        $user_email = !empty($_POST['user_email']) ? cm_pn_forms::customers_manager_pn_sanitizer(wp_unslash($_POST['user_email'])) : 0;
                  ...
                  do_action('cm_pn_form_save', $user_id, $customers_manager_pn_key_value, $cm_pn_form_type, $cm_pn_form_subtype);
                  break;
                case 'post':
                  if (empty($cm_pn_form_subtype) || in_array($cm_pn_form_subtype, ['post_new', 'post_edit'])) {
                    if (empty($post_id)) {
                      if (CUSTOMERS_MANAGER_PN_Functions_User::customers_manager_pn_user_is_admin(get_current_user_id())) {
                        ...
                        $title = !empty($_POST[$post_type . '_title']) ? cm_pn_forms::customers_manager_pn_sanitizer(wp_unslash($_POST[$post_type . '_title'])) : '';
                        $description = !empty($_POST[$post_type . '_description']) ? cm_pn_forms::customers_manager_pn_sanitizer(wp_unslash($_POST[$post_type . '_description'])) : '';
                  ...
                  do_action('cm_pn_form_save', $post_id, $customers_manager_pn_key_value, $cm_pn_form_type, $cm_pn_form_subtype, $post_type);
                  ...
                  if (!empty($post_type) && $post_type === 'cm_pn_org') {
                     ...
                     */
                    do_action('cm_pn_org_form_save', $post_id, $customers_manager_pn_key_value, $cm_pn_form_type, $cm_pn_form_subtype);
                  }
                  break;
                case 'option':
                  if (CUSTOMERS_MANAGER_PN_Functions_User::customers_manager_pn_user_is_admin(get_current_user_id())) {
                  ...
                  do_action('cm_pn_form_save', 0, $customers_manager_pn_key_value, $cm_pn_form_type, $cm_pn_form_subtype);
                  break;
              }
