(function ($) {
  ...
  $(document).ready(function () {
    ...
    $(document).on('click', '.pn-customers-manager-popup-open-ajax', function (e) {
      ...
      // Try alt-id attribute (used by contacts section)
      if (!pn_cm_organization_id) {
        pn_cm_organization_id = pn_customers_manager_btn.attr('data-pn_cm_organization_alt-id') || '';
      }
    ...
    // Contact add submit handler
    $(document).on('click', '.pn-customers-manager-contact-add-submit', function (e) {
      e.preventDefault();
      var $btn = $(this);
      var orgId = $btn.attr('data-org-id');
      var userId = $btn.closest('.pn-customers-manager-contact-add-form').find('#pn_cm_contact_user_id').val();

      if (!userId) {
        alert(pn_customers_manager_i18n.an_error_has_occurred || 'Please select a user.');
        return;
      }

      $btn.addClass('pn-customers-manager-link-disabled');
      ...
      $.ajax({
        ...
        data: {
          action: 'pn_customers_manager_ajax',
          pn_customers_manager_ajax_type: 'pn_customers_manager_contact_add',
          pn_customers_manager_ajax_nonce: pn_customers_manager_ajax.pn_customers_manager_ajax_nonce,
          pn_cm_organization_id: orgId,
          pn_cm_contact_user_id: userId
        },
        success: function (response) {
          var data = typeof response === 'object' ? response : JSON.parse(response);
          if (data.error_key) {
            pn_customers_manager_get_main_message(data.error_content || pn_customers_manager_i18n.an_error_has_occurred);
            $btn.removeClass('pn-customers-manager-link-disabled');
            return;
          }
          // Update contacts list in the page
          var $contactsList = $('.pn-customers-manager-organization-contacts-field[data-pn_cm_organization_alt-id="' + orgId + '"] .pn-customers-manager-organization-contacts-list');
          if ($contactsList.length && data.contacts_html) {
            $contactsList.html(data.contacts_html);
          }
          pn_customers_manager_get_main_message(pn_customers_manager_i18n.saved_successfully);
          pn_customers_manager_Popups.close();
        },
    ...
    $(document).on('click', '.pn-customers-manager-remove-contact', function (e) {
      e.preventDefault();
      var $btn = $(this);
      var contactId = $btn.attr('data-contact-id');
      var orgId = $btn.attr('data-org-alt-id');
      ...
      $.ajax({
        ...
        data: {
          action: 'pn_customers_manager_ajax',
          pn_customers_manager_ajax_type: 'pn_customers_manager_contact_remove',
          pn_customers_manager_ajax_nonce: pn_customers_manager_ajax.pn_customers_manager_ajax_nonce,
          pn_cm_organization_id: orgId,
          pn_cm_contact_user_id: contactId
        },
        success: function (response) {
          var data = typeof response === 'object' ? response : JSON.parse(response);
          if (data.error_key) {
          ...
          // Update contacts list in the page
          var $contactsList = $btn.closest('.pn-customers-manager-organization-contacts-list');
          if ($contactsList.length && data.contacts_html) {
            $contactsList.html(data.contacts_html);
          }
          pn_customers_manager_get_main_message(pn_customers_manager_i18n.removed_successfully || pn_customers_manager_i18n.saved_successfully);
        },
        error: function () {
          pn_customers_manager_get_main_message(pn_customers_manager_i18n.an_error_has_occurred);
          $btn.removeClass('pn-customers-manager-link-disabled');
